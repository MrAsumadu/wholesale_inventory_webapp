...
import { inventoryItems, categories, shops, orders } from "@/lib/mock-data";
  ...
  return (
    <div className="p-4 md:p-8 max-w-[1200px] mx-auto animate-fade-in-up">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8 stagger-children">
        ...
        {/* Low Stock Alerts */}
        <Card className={lowStockItems.length > 0 ? "border-amber-200 bg-amber-50/30" : "border-border"}>
          <CardContent className="p-5">
            <div className="flex items-center justify-between mb-3">
              <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${lowStockItems.length > 0 ? "bg-amber-100" : "bg-muted"}`}>
                <AlertTriangle className={`w-5 h-5 ${lowStockItems.length > 0 ? "text-amber-600" : "text-muted-foreground"}`} />
              </div>
          ...
          <Card className="border-border">
            <CardContent className="p-0">
              {recentOrders.map((order, i) => (
                <Link key={order.id} href={`/shops/${order.shopId}`}>
                  <div
